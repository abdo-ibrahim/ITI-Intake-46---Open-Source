...
const { decreaseStock } = productsStore;
...
function handleBuyNow() {
  if (props.product.stock > 0) {
    decreaseStock(props.product.id);
...
<template>
  <article class="flex flex-col gap-3 p-4 border border-slate-200 rounded-xl bg-white shadow-sm hover:-translate-y-1 hover:shadow-lg transition duration-200">
    ...
    <button :disabled="props.product.stock <= 0" :class="getCTAClass" class="mt-1" @click="handleBuyNow">Buy Now</button>
  </article>
</template>
