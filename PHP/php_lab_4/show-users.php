...
$currentUserName = isset($_SESSION['user']['name']) ? htmlspecialchars($_SESSION['user']['name']) : 'User';
?>
...
<body class="bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100 min-h-screen py-10">
  <div class="max-w-6xl mx-auto bg-white/90 p-8 rounded-2xl shadow-2xl border border-blue-200">
    <div class="flex flex-col gap-4 md:flex-row md:justify-between md:items-center mb-6">
      <div>
        <h1 class="text-4xl font-extrabold text-blue-700 tracking-tight">User List</h1>
        <p class="text-slate-600 mt-2">Welcome, <?php echo $currentUserName; ?></p>
      </div>
      <div class="flex flex-wrap gap-3">
        ...
        <a href="logout.php" class="inline-block bg-slate-700 hover:bg-slate-900 text-white font-semibold py-2 px-4 rounded shadow transition">Logout</a>
      </div>
    </div>
        ...
        <tbody class="bg-white divide-y divide-blue-100">
          ...
          if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              ...
              echo "</td>";
              echo "<td class='py-2 px-4 space-x-2 whitespace-nowrap'>";
              ...
              echo "<a href=\"edit-user.php?id={$row['id']}\" class='inline-block bg-amber-500 hover:bg-amber-600 text-white font-semibold py-1 px-3 rounded shadow transition'>Edit</a>";
